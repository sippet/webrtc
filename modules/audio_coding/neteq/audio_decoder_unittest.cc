...
TEST_F(AudioDecoderG729Test, EncodeDecode) {
  ...
  EncodeDecodeTest(data_length_ / frame_size_ * 10, tolerance, mse, delay);
  ReInitTest();
  EXPECT_TRUE(decoder_->HasDecodePlc());
}
...
const bool has_g729 = true;
#else
const bool has_g729 = false;
#endif
}  // namespace
...
TEST(AudioDecoder, CodecSampleRateHz) {
  ...
  EXPECT_EQ(has_g729 ? 8000 : -1, CodecSampleRateHz(kDecoderG729));
...
TEST(AudioDecoder, CodecSupported) {
  ...
  EXPECT_EQ(has_g729, CodecSupported(kDecoderG729));
